...
class RoomClassifierTrainingDataGenerator:
  ...
  def __init__(self, gen_flavour, datasets_to_gen = 1000):
    ...
    if gen_flavour == ModelType.FEATURES_12:
        ...
        room_types = ['bedroom', 'bathroom', 'living room', 'kitchen'] # With the detectable models that we have, we'll be able to detect these rooms.

        self.convert_ai2_thor_training_data(gen_flavour, room_types, items_to_remove, items_to_convert, converting_to)
    elif gen_flavour == ModelType.AI2_THOR_SURR:
        # So we have these features - that's all we know how to detect.
        # Let's construct a few labels (room types) and assign some of these items to them
        room_types = ['bedroom', 'bathroom', 'kitchen', 'living room']

        possible_items_in_each_room = [
        (room_types[0], ['alarmclock', 'apple', 'bed', 'blinds', 'book', 'box',
                         'candle', 'cd', 'cellphone', 'chair', 'cloth',
                         'creditcard', 'cup', 'curtains', 'desk', 'desklamp',
                         'desktop', 'dogbed', 'drawer', 'dresser', 'dumbbell',
                         'floor', 'floorlamp', 'houseplant', 'keychain', 'laptop',
                         'lightswitch', 'mirror', 'mug', 'newspaper', 'ottoman',
                         'painting', 'pen', 'pencil', 'pillow', 'poster',
                         'roomdecor', 'shelf', 'shelvingunit', 'sidetable',
                         'statue', 'teddybear', 'tissuebox', 'vase', 'watch',
                         'window', 'winebottle']),
        (room_types[1], ['bathtub', 'bathtubbasin', 'bottle', 'cabinet', 'candle',
                         'cloth', 'faucet', 'floor', 'garbagecan', 'houseplant', 'laundryhamper',
                         'lightswitch', 'mirror', 'plunger', 'scrubbrush',
                         'shelf', 'showercurtain', 'showerdoor', 'showerglass',
                         'showerhead', 'sinkbasin', 'soapbar', 'soapbottle',
                         'spraybottle', 'toilet', 'toiletpaper', 'toiletpaperhanger',
                         'towel', 'towelholder', 'window']),
        (room_types[2], ['aluminumfoil', 'apple', 'blinds', 'book', 'bottle',
                         'bowl', 'box', 'bread', 'butterknife', 'cabinet',
                         'cellphone', 'cloth', 'coffeemachine', 'countertop',
                         'cup', 'diningtable', 'dishsponge', 'drawer', 'egg',
                         'faucet', 'floor', 'fork', 'fridge', 'garbagebag',
                         'garbagecan', 'handtowel', 'handtowelholder', 'kettle',
                         'knife', 'ladle', 'lettuce', 'lightswitch', 'microwave',
                         'mug', 'pan', 'papertowelroll', 'pen', 'pencil',
                         'peppershaker', 'plate', 'pot', 'potato', 'saltshaker',
                         'scrubbrush', 'shelf', 'sink', 'soapbar', 'soapbottle',
                         'spatula', 'spoon', 'spraybottle', 'stool', 'stoveburner',
                         'stoveknob', 'tabletopdecor', 'toaster', 'tomato', 'towel',
                         'towelholder', 'window', 'winebottle']),
        (room_types[3], ['apple', 'armchair', 'baseballbat', 'basketball',
                         'blinds', 'book', 'bottle', 'box', 'candle', 'cd',
                         'cellphone', 'chair', 'coffeetable', 'creditcard', 'cup',
                         'curtains', 'diningtable', 'drawer', 'dumbbell', 'floor',
                         'floorlamp', 'footstool', 'garbagebag', 'houseplant',
                         'keychain', 'laptop', 'lightswitch', 'mirror', 'mug',
                         'newspaper', 'ottoman', 'painting', 'pen', 'pencil',
                         'poster', 'remotecontrol', 'roomdecor', 'safe', 'shelf',
                         'shelvingunit', 'sidetable', 'sofa', 'statue', 'teddybear',
                         'television', 'tennisracket', 'tissuebox', 'tvstand',
                         'vacuumcleaner', 'vase', 'watch', 'window', 'winebottle'])
        ]
        ...
        file = open("pkl/features_for_each_label_AI2_THOR_12.pkl",'rb')
        features_for_each_label_ai2thor = pickle.load(file)
        file.close()

        file = open("pkl/labels_shuffled_AI2_THOR_12.pkl",'rb')
        labels_shuffled_ai2thor = pickle.load(file)
        file.close()

        file = open("pkl/features_for_each_label_FEATURES_12.pkl",'rb')
        features_for_each_label_gen_scannet = pickle.load(file)
        file.close()

        file = open("pkl/labels_shuffled_FEATURES_12.pkl",'rb')
        ...
        pickle.dump(combined_labels, open("pkl/labels_shuffled_" + gen_flavour.name + ".pkl", "wb"))
        pickle.dump(combined_features, open("pkl/features_for_each_label_" + gen_flavour.name + ".pkl", "wb"))
        print("Stored " + str(len(combined_labels)) + " samples for " + gen_flavour.name)
    elif gen_flavour == ModelType.HYBRID_AT_18:
        file = open("pkl/features_for_each_label_AI2_THOR_18.pkl",'rb')
        features_for_each_label_ai2thor = pickle.load(file)
        file.close()

        file = open("pkl/labels_shuffled_AI2_THOR_18.pkl",'rb')
        labels_shuffled_ai2thor = pickle.load(file)
        file.close()

        file = open("pkl/features_for_each_label_FEATURES_18.pkl",'rb')
        features_for_each_label_gen_scannet = pickle.load(file)
        file.close()

        file = open("pkl/labels_shuffled_FEATURES_18.pkl",'rb')
        labels_shuffled_gen_scannet = pickle.load(file)
        file.close()
        ...
        pickle.dump(combined_features, open("pkl/features_for_each_label_" + gen_flavour.name + ".pkl", "wb"))
        print("Stored " + str(len(combined_labels)) + " samples for " + gen_flavour.name)
    elif gen_flavour == ModelType.HYBRID_AI2_THOR:
        file = open("pkl/features_for_each_label_AI2_THOR_SURR.pkl",'rb')
        features_for_each_label_surr_ai2thor = pickle.load(file)
        file.close()

        file = open("pkl/labels_shuffled_AI2_THOR_SURR.pkl",'rb')
        labels_shuffled_surr_ai2thor = pickle.load(file)
        file.close()

        file = open("pkl/features_for_each_label_AI2_THOR.pkl",'rb')
        features_for_each_label_orig_ai2thor = pickle.load(file)
        file.close()

        file = open("pkl/labels_shuffled_AI2_THOR.pkl",'rb')
        labels_shuffled_orig_ai2thor = pickle.load(file)
        file.close()

        combined_labels = labels_shuffled_surr_ai2thor + labels_shuffled_orig_ai2thor
        combined_features = features_for_each_label_surr_ai2thor + features_for_each_label_orig_ai2thor
        #print("Storing: " + "labels_shuffled_" + gen_flavour.name + ".pkl")
        pickle.dump(combined_labels, open("pkl/labels_shuffled_" + gen_flavour.name + ".pkl", "wb"))
        pickle.dump(combined_features, open("pkl/features_for_each_label_" + gen_flavour.name + ".pkl", "wb"))
        print("Stored " + str(len(combined_labels)) + " samples for " + gen_flavour.name)
  ...
  def convert_ai2_thor_training_data(self, gen_flavour, room_types, items_to_remove, items_to_convert, converting_to):
    ...
    pickle.dump(labels_filtered, open("pkl/labels_shuffled_" + gen_flavour.name + ".pkl", "wb"))
    pickle.dump(features_for_each_label_filtered, open("pkl/features_for_each_label_" + gen_flavour.name + ".pkl", "wb"))
  ...
  def generate_training_data(self, gen_flavour, room_types, possible_items_in_each_room):
    ...
    labels_fname = "pkl/labels_shuffled_" + gen_flavour.name + ".pkl"
    features_fname = "pkl/features_for_each_label_" + gen_flavour.name + ".pkl"
...
def generateHybridAI2Thor(number_of_datasets_to_gen):
    gen = RoomClassifierTrainingDataGenerator(ModelType.AI2_THOR_SURR, number_of_datasets_to_gen)
    gen = RoomClassifierTrainingDataGenerator(ModelType.HYBRID_AI2_THOR)

def main():
    #generateHybridAI2Thor_Scannet()
    #generateScannet()
    generateHybridAI2Thor(1000)
